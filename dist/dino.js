...
let resetImg;
// Physics
let velocityX = -8; // Vitesse de déplacement des cactus vers la gauche
let velocityY = 0;
let gravity = 0.4;
let gameOver = false;
let score = 0;
// Nouvelles variables pour les scores avec types
let playerScores = []; // Array temporaire pour stocker les scores récupérés
let currentPlayerName = "Joueur"; // Nom du joueur actuel
let nameIsConfigured = false; // Vrai si un nom est déjà enregistré
let topScoresToDisplay = []; // Top 3 scores à afficher
let currentScreen = "gameOver"; // Quel écran afficher après game over
let playerNameInput = ""; // Texte saisi par le joueur pour entrer son nom
let lastScore = 0; // Score de la dernière partie
...
function drawScoresScreen() {
    ...
    if (topScoresToDisplay.length === 0) {
        ...
        topScoresToDisplay.forEach((entry, index) => {
            const yPos = boardHeight / 2 + 35 + (index + 1) * 35;
